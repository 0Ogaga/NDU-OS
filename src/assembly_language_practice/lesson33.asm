...
_fork:
    ; --- 5. FORK THE PROCESS (SYS_FORK, RAX=57) ---
    mov     rax, 57            ; RAX = SYS_FORK (replaces 32-bit opcode 2)
    syscall                    ; Returns PID of child in RAX (parent) or 0 (child)
    
    cmp     rax, 0             ; if return value of SYS_FORK in RAX is zero we are in the child process
    je      _read              ; jmp in child process to _read
    
    ; --- PARENT PROCESS ---
    jmp     _accept            ; jmp in parent process to _accept (to wait for next client)

_read:
    ; --- CHILD PROCESS: HANDLE CONNECTION ---
    
    ; --- 6. READ FROM THE CLIENT SOCKET (SYS_READ, RAX=0) ---
    
    ; Argument 1: File Descriptor
    mov     rdi, r13           ; RDI = Client FD (from R13)
    
    ; Argument 2: Buffer Address
    mov     rsi, buffer        ; RSI = address of our buffer
    
    ; Argument 3: Count (size to read)
    mov     rdx, 255           ; RDX = 255 bytes max
    
    mov     rax, 0             ; RAX = SYS_READ (replaces 32-bit opcode 3)
    syscall                    ; Returns bytes read in RAX.
    
    ; --- PRINT THE REQUEST ---
    mov     rdx, rax           ; RDX = length of bytes read
    mov     rsi, buffer        ; RSI = address of the string to print
    mov     rax, 1             ; RAX = SYS_WRITE to print the request to STDOUT
    mov     rdi, 1             ; RDI = STDOUT FD
    syscall
    
    ; The original lesson implicitly assumes the child process exits after this.
    ; To prevent resource leaks and follow the implicit pattern:
    
    ; Clean up the client FD before exiting the child.
    mov     rdi, r13           ; RDI = Client FD
